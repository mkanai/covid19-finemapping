...
main <- function(args) {
  df <- data.table::fread(args$z)
  n <- args$n_samples
  L <- args$L

  if (!is.null(args$prior_weights)) {
    prior_weights <- data.table::fread(args$prior_weights)
    df$v <- stringr::str_c(df$chromosome, df$position, df$allele1, df$allele2, sep = ":")
  ...
  if (!is.null(args$ld)) {
    ...
    # require(Matrix)
    # R <- as.matrix(Matrix::Diagonal(nrow(df)))
  ...
  if (args$compute_yty) {
    ...
    pheno <- data.table::fread(args$pheno)
    var_y <- var(pheno[, 0])
  ...
  if (args$L > 1) {
    ...
    variables <- dplyr::select(variables, -cs_specific_prob)
  }
  if (is.null(args$ld)) {
    variables <- dplyr::select(variables, -dplyr::one_of("mean", "sd", "low_purity", "lead_r2"))
  }
